<template>
  <div class="answer">
    ...
    <div class="answer_img">
      <template v-if="data.files && isArray(data.files)">
        <img :src="data.files[1].url" alt="" />
      </template>
    </div>
    <div
      class="item"
    ...
    <div class="area_0">
      <div class="area">
        <div
          ...
          @right="i == data.answers.length && onStopMove($event)"
        >
...
<script>
import { log, getRandomInt, debounce, delayedAction } from '../tool/index.js'
import {
  handleAndroidvClick,
  handleAndroidvBack,
  AndroidLocal
...
export default {
  data() {
    return {
      ...
      // 题目索引
      curAnswerIndex: !this.$route.query.curAnswerIndex
        ? 0
        : parseInt(this.$route.query.curAnswerIndex),
      isCorrect: null,
      data: {},
  ...
  methods: {
    ...
    init() {
      this.analysisUrl()
      new CountDown(this.time, { interval: 1000 }, ({ ss, SSS }) => {
        this.countDown = `${ss}:${SSS.slice(0, 2)}`
      })
    },
    async getData() {
      ...
      const findData = data[this.curAnswerIndex]
      this.data = findData
      this.lock = true
      this.score = findData.score
      this.title = findData.title
      this.isCorrect = findData.answers.findIndex((item) => item.is_right == 1)
      return data
    },
    async analysisUrl() {
      AndroidLocal('assetId', this.assetId)
      await this.getData()
    },
    // 定位到area1
    focusPositionArea1() {
    ...
    startSuccessAnimation() {
      ...
      const successTimer5 = delayedAction(() => {
        ...
        AndroidLocal(`subject${this.curAnswerIndex}`, '1')
    ...
    autoCloseSuccessModal() {
      ...
      this.timer = delayedAction(() => {
        this.answer_success.state == 'stop' && handleAndroidvBack()
      }, this.autoCloseTime)
    },
    // 重置回答错误弹窗
    resetError() {
    ...
    startErrorAnimation() {
      ...
      const errorTimer2 = delayedAction(() => {
        ...
        AndroidLocal(`subject${this.curAnswerIndex}`, '0')
    ...
    autoCloseErrorModal() {
      this.clearTimeout()
      this.timer = delayedAction(() => {
        this.answer_error.state == 'stop' && handleAndroidvBack()
      }, this.autoCloseTime)
    },
    // 清除计时器
    clearTimers() {
    ...
    handleLock() {
      if (!this.lock) return
      this.lock = false
      this.playAudioInit(this.data.files[0].url)
    },
    hangleAudioLock() {
    ...
    onArea0Focus: debounce(function (index) {
      this.playAudio(this.data.answers[index].files[0].url)
    }, debounceTime),
    onArea1Focus() {
  ...
  watch: {
    countDown(newVal, oldVal) {
      if (newVal <= '00:00') handleAndroidvBack()
    }
  },
  created() {
    this.init()
  ...
  mounted() {
    document.onkeydown = debounce((key) => {
      ...
      if (code == 13) {
        key.preventDefault()
        if (this.answer_success.state == 'stop') return handleAndroidvBack()
        if (this.answer_error.state == 'stop') return handleAndroidvBack()
        ...
        if (focus) {
          ...
          if (!focus.dataset.methods === false) {
            switch (focus.dataset.methods) {
              case 'onClick':
                if (index == this.isCorrect) {
                  addBeans({
                    question_id: this.data.id,
                    answer_id: this.data.answers[this.isCorrect].id
                  })
