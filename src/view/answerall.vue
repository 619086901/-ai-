...
import { getQuestions } from '../api/problem.js'
...
export default {
  data() {
    return {
      ...
      list: [
        { id: 1, name: '颜色', sum: 0 },
        { id: 2, name: '逻辑思维', sum: 0 },
        { id: 3, name: '观察记忆', sum: 0 },
        { id: 4, name: '图形', sum: 0 },
        { id: 5, name: '数感', sum: 0 }
      ]
    }
  ...
  methods: {
    ...
    async getData() {
      ...
      data.forEach((item) => {
        this.list.forEach((x) => {
          if (x.sum < 4 && x.name == item.category) {
            this.data.push({
              ...item,
              rightKey: item.answers.findIndex((item) => item.is_right == 1),
    ...
    startSuccessAnimation() {
      ...
      // this.startSuccessAnimationTimer()
    ...
    document.onkeydown = debounce((key) => {
      ...
      if (this.successPro) {
        this.$router.push({
          ...
          query: {
            sum: this.answer_success.score.state,
